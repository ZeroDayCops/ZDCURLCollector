import urllib.request
import urllib.error
import json
import sys
import os
from dotenv import load_dotenv

load_dotenv()

def test_apify():
    token = os.getenv("APIFY_TOKEN", "").strip() or "YOUR_APIFY_TOKEN_HERE"
    if token == "YOUR_APIFY_TOKEN_HERE" or not token:
        print("❌ Error: APIFY_TOKEN is not set in environment or .env file.")
        return
    username = "trendsofindia_"
    
    url = f"https://api.apify.com/v2/acts/apify~instagram-post-scraper/run-sync-get-dataset-items?token={token}"
    
    payload = {
        "username": [username],
        "resultsLimit": 3,
        "enhanceUserAreaWithStatus": False
    }
    
    data_bytes = json.dumps(payload).encode('utf-8')
    req = urllib.request.Request(
        url,
        data=data_bytes,
        headers={'Content-Type': 'application/json'}
    )
    
    print(f"🚀 Running Apify Instagram Scraper for '{username}'...")
    try:
        with urllib.request.urlopen(req, timeout=120) as response:
            status_code = response.getcode()
            print(f"Status Code: {status_code}")
            
            res_body = response.read().decode('utf-8')
            data = json.loads(res_body)
            
            print(f"✅ Success! Found {len(data)} items:")
            for idx, item in enumerate(data, 1):
                post_url = item.get("url")
                caption = item.get("caption", "")[:60]
                posted_at = item.get("timestamp")
                print(f"  [{idx}] URL: {post_url}")
                print(f"      Caption: {caption}")
                print(f"      Posted: {posted_at}")
    except urllib.error.HTTPError as e:
        print(f"❌ HTTP Error {e.code}: {e.read().decode('utf-8', errors='ignore')}")
    except Exception as e:
        print(f"❌ Exception: {e}")

if __name__ == "__main__":
    test_apify()
